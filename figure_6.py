...
# #------------------------------Bar plot, figure 6 panel f  ----------------------------------------
# best_weights=pickle.load(open(semi_path+date_aniso+'\\'+type_aniso+'\\_best_weights.p','rb'))
best_weights_fep=pickle.load(open(semi_path_fep+'20210223-160846_aniso\\'+'_best_weights.p','rb'))
...
for i in range(best_weights.shape[0]) :
    ...
    best_weights_i =best_weights[i,:]+best_weights_fep[i,:]#  for OSA
...
plt.show()
fig_bar.savefig(save_path+'bar_plot_total.png',dpi=300, bbox_inches='tight')
# #
# # ##-----------------------------------Total images, figure 6 panel a -------------------------------
# ###Corrected
total_im_corr= np.array(Image.open(path_images+'after_correction.tif'))
...
####Raw
total_im_raw= np.array(Image.open(path_images+'before_correction.tif'))
#max and min corrected raw
...
#-------------------------------Fep correction
####Raw
total_im_fep= np.array(Image.open(path_images+'fep_correction.tif'))
#max and min corrected raw
min_total_fep=np.amin(total_im_fep)
max_total_fep=np.amax(total_im_fep)
print('max/min raw total',min_total_fep, max_total_fep)

# --Normalization
total_im_fep_norm=(total_im_fep)/(max_total_corr)
# total_im_raw_norm_plot=(total_im_raw_norm)
total_im_fep_norm_plot=total_im_fep_norm[800:1000,650:1450]
#max and min corrected normalized
# # im_corr_norm_plot=im_corr_norm[70:120,70:120]
min_total_im_fep_norm=np.amin(total_im_fep_norm_plot)
max_total_im_fep_norm=np.amax(total_im_fep_norm_plot)
print(' min/max raw total norm', min_total_im_fep_norm, max_total_im_fep_norm)
#
#
...
fig_2=plt.figure('Corr fep image', figsize=(10, 4))
# plot_aberrated=plt.imshow(im_aberrated,cmap = 'jet', )
plot_fep=plt.imshow(total_im_fep_norm_plot, vmin=0, vmax=1,cmap = 'jet', )
scalebar = ScaleBar(0.195, 'um', frameon=False,color='white',location='lower right',scale_formatter=lambda value, unit: f'')
plt.gca().add_artist(scalebar)
cbar_ab = plt.colorbar(plot_fep, ticks=[0, 1], orientation="vertical", fraction=0.012, pad=0.01)
#cbar_ab.set_ticklabels([r'$161$', r'$13559$'])
cbar_ab.set_ticklabels([r'$0$', r'$1$'])
plt.axis('off')
plt.show()
fig_2.savefig(save_path+'im_total_fep_norm.png',dpi=300, bbox_inches='tight')

#figure corrected image
fig_3=plt.figure('Corr image', figsize=(10, 4))
# plot_aberrated=plt.imshow(im_aberrated,cmap = 'jet', )
...
plt.axis('off')
plt.show()
fig_3.savefig(save_path+'im_total_corr_norm.png',dpi=300, bbox_inches='tight')
...
folders=os.listdir(path_images+sub_folder)
...
# rois_files=fnmatch.filter(os.listdir('M:\\tnw\\ist\\do\\projects\\Zebrafish\\DATA\\Laura\\experiments_AO\\02_02\\beads_0.1_um\\set_2\\stack_20210202-175600\\4analysis\\'), '*.tif')
    ...
    #---------------------Plot xy focus, figure 6 panel b---------------------------------------------
...
plt.show()
fig_int_max.savefig(save_path+'lateral_intensities_norm_max.png',dpi=300, bbox_inches='tight')
...
# coords_ref_temp=np.load(path_images+"Final_coords_holoName.npy")
...
# coords_corr_im=np.load(path_images+"centers_corr_cgh_bis.npy")
# coords_raw_im=np.load(path_images+"centers_raw_cgh_bis.npy")
#
...
diameter_fep =np.zeros((len(points),41), dtype=np.float)
intensity_fep =np.zeros((len(points),41), dtype=np.float)
for j, point  in enumerate (points):
    ...
    for nn in number:
        ...
        # intensity_temp[i] = np.amax(image)
        ...
        i += 1
    intensity_fep[j,:] = intensity_temp/np.amax(intensity_temp)
    diameter_fep[j,:]  = diameter_temp/np.amax(diameter_temp)
    metric_1_fep[j,:] = metric_1_temp/np.amax(metric_1_temp)
...
#
# plt.show()
# fig_d.savefig(save_path+'fep_dimeter_improvement.png', dpi=300, bbox_inches='tight')
# fig_int.savefig(save_path+'fep_intensity_improvement.png', dpi=300, bbox_inches='tight')
...
#     axs[1].plot(x,intensity_fepfish[j,:],'o-', color=color_i[j],label='ROI '+str(j+1))
#
#
#     # axs[i].plot(results_FWHM_fit_raw[i][0],0.5*(np.amax(results_fit_raw[i])-np.amin(results_fit_raw[i]))*np.ones(np.shape(results_FWHM_fit_raw[i][0])),color='darkorange',linestyle='dashed')
#     # axs[i].plot(results_FWHM_fit_corr[i][0],0.5*(np.amax(results_fit_corr[i])-np.amin(results_fit_corr[i]))*np.ones(np.shape(results_FWHM_fit_corr[i][0])),color='darkblue',linestyle='dashed')
...
fig_m1_vs_index, axs = plt.subplots(2,1,figsize=(15, 7))
fig_m1_vs_index.subplots_adjust(hspace = .6, wspace=.3)
axs = axs.ravel()
x = np.linspace(3,44,41)
for  j in range(len(points)):


    axs[0].plot(x,metric_1_fep[j,:],'o-', color=color_i[j], label='ROI '+str(j+1))
    axs[1].plot(x,metric_1_fepfish[j,:],'o-', color=color_i[j],label='ROI '+str(j+1))
...
for ax in axs.flat:
    ax.label_outer()
fig_m1_vs_index.text(0.5, 0.00, r'$Zernike \quad index $', ha='center', fontsize=MEDIUM_SIZE)
fig_m1_vs_index.text(0.04, 0.5, r'$Normalized \quad M_1 $', va='center', rotation='vertical',fontsize=MEDIUM_SIZE)
# plt.legend(loc='lower right', mode="expand", ncol=4)
axs[1].legend(loc='lower center', bbox_to_anchor=(0.3, -1),fancybox=False, shadow=False, ncol=4, fontsize=MEDIUM_SIZE)
plt.show()
...
metric_1_fep=np.zeros((len(points),n_meas), dtype=np.float)
diameter_fep=np.zeros((len(points),n_meas), dtype=np.float)
intensity_fep =np.zeros((len(points),n_meas), dtype=np.float)
...
fig_m1_vs_meas, axs = plt.subplots(2,1,figsize=(15, 7))
fig_m1_vs_meas.subplots_adjust(hspace = .6, wspace=.3)
axs = axs.ravel()
...
for ax in axs.flat:
    ax.label_outer()
fig_m1_vs_meas.text(0.5, 0.00, r'$Iterations $', ha='center', fontsize=MEDIUM_SIZE)
fig_m1_vs_meas.text(0.04, 0.5, r'$Normalized \quad M_1 $', va='center', rotation='vertical',fontsize=MEDIUM_SIZE)
# plt.legend(loc='lower right', mode="expand", ncol=4)
axs[1].legend(loc='lower center', bbox_to_anchor=(0.3, -1),fancybox=False, shadow=False, ncol=4, fontsize=MEDIUM_SIZE)
plt.show()
